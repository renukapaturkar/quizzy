    ...
    (async function () {
      try {
        const response = await axios.get(
          `https://quizzy-backend.renukapaturkar.repl.co/quiz/${id}`
        );
            ...
            {scoreBoard && (
              <div className="self-center p-2">
                <input
                  ...
                  className="border text-md self-center p-2"
                  onChange={(e) => setName(e.target.value)}
                  autoFocus
                />
                <button
                  className="border bg-blue-800 text-white rounded-md text-sm p-2"
                  onClick={() => saveToScoreBoard()}
                >
                  Save
                </button>
              </div>
        ...
        <Results />
          ...
          <div className="flex flex-col w-full md:max-w-lg h-auto p-8 self-center border shadow-lg">
            ...
            <h1 className="text-3xl text-center font-semibold text-gray-800">
              Question {data?.questions[currentQuestion].questionNo}/
            ...
            <div className="flex justify-end p-2 text-blue-900 font-semibold">
              ...
              <button className="p-4 text-2xl" onClick={currentQuestionHandler}>
                Next
              </button>
            </div>
          </div>
